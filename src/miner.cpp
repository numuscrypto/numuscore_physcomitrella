...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("NUMUSMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("NUMUSMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("NUMUSMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("numus-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running NUMUSMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
