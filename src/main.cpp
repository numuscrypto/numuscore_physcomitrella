...
#if defined(NDEBUG)
#error "NUMUS cannot be compiled without assertions."
#endif
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos)
{
    ...
    if (block.IsProofOfWork()) {
        ...
        if (block.nTime > 1542847405 && !CheckProofOfWork(block.GetHash(), block.nBits))
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        if (nHeight < 200 && nHeight > 0)
            return 1000 * COIN;
                    ...
                    else if (nHeight > 600 * 1000 && nHeight-1 <= 655 * 1000) {
                        nSubsidy = 0.75 * COIN;
                    }
                    else if (nHeight-1 > 655 * 1000 && nHeight <= 657 * 1000) {
                        nSubsidy = 0.5 * COIN;
                    }
                    ...
                    else if (nHeight > 685 * 1000 && nHeight <= 700 * 1000) {
                        nSubsidy = 30 * COIN;
                    }
                    else if (nHeight > 700 * 1000 && nHeight <= 750 * 1000) {
                        nSubsidy = 20 * COIN;
                    }
                    else if (nHeight > 750 * 1000 && nHeight <= 800 * 1000) {
                        nSubsidy = 20 * COIN;
                    }
                    else if (nHeight > 800 * 1000 && nHeight <= 1000 * 1000) {
                        nSubsidy = 10 * COIN;
                    }
                    else if (nHeight > 1000 * 1000 && nHeight <= 1300 * 1000) {
                        nSubsidy = 8 * COIN;
                    }
                    else if (nHeight > 1300 * 1000 && nHeight <= 2000 * 1000) {
                        nSubsidy = 5 * COIN;
                    }
                    else if (nHeight > 2000 * 1000 && nHeight <= 3000 * 1000) {
                        nSubsidy = 3 * COIN;
                    }
                    else {
                    nSubsidy = 1 * COIN;   
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight <= 655000) {
        ret = blockValue * 3/4;
    } else if (nHeight > 655000 && nHeight <= 660000) {
        ret = blockValue * 3/10;
    } else if (nHeight > 660000 && nHeight <= 665000) {
        ret = blockValue * 4/10;
    } else if (nHeight > 665000 && nHeight <= 670000) {
        ret = blockValue * 5/10;
    } else if (nHeight > 670000 && nHeight <= 685000) {
        ret = blockValue * 6/10;
    } else {
    ret = blockValue * 7/10;        
    }
...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                  pindexBestHeader->GetBlockTime() < GetTime() - 48 * 60 * 60); // ~144 blocks behind -> 2 x fork detection time
    if (!state)
        lockIBDState = true;
    return state;
}
...
bool CScriptCheck::operator()()
{
    ...
    if (!VerifyScript(scriptSig, scriptPubKey, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, cacheStore), &error)) {
        ...
        //LogPrintf("Script check error on block %s \n", ptxTo->GetHash().ToString());
    }
    return true;
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (pindex->nHeight > 655035 && pindex->nHeight <= Params().LAST_POW_BLOCK() && block.IsProofOfStake())
        return state.DoS(100, error("ConnectBlock() : PoS period not active"),
            REJECT_INVALID, "PoS-early");

    //if (pindex->nHeight < 400000 pindex->nHeight > Params().LAST_POW_BLOCK() && block.IsProofOfWork())
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (block.GetBlockTime() > 1542951259 && fCheckPOW && !CheckProofOfWork(block.GetHash(), block.nBits))
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    //LogPrintf("Limits Is OK \n");
    // First transaction must be coinbase, the rest must not be
    if (block.vtx.empty() || !block.vtx[0].IsCoinBase())
        return state.DoS(100, error("CheckBlock() : first tx is not coinbase"),
            REJECT_INVALID, "bad-cb-missing");
    //LogPrintf("1st tx Is OK \n");
    for (unsigned int i = 1; i < block.vtx.size(); i++)
        if (block.vtx[i].IsCoinBase())
            return state.DoS(100, error("CheckBlock() : more than one coinbase"),
                REJECT_INVALID, "bad-cb-multiple");
        //LogPrintf("Coinbase double Is OK \n");
    if (block.IsProofOfStake()) {
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            //LogPrintf("Starting MN payment checks");
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (pindexPrev->nHeight + 1 > 655250 && block.nBits != nBitsRequired)
    ...
    if (block.IsProofOfStake()) {
        ...
        if(pindexPrev->nHeight + 1 > 655250 && !CheckProofOfStake(block, hashProofOfStake)) {
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (nHeight > 655050 && block.GetBlockTime() <= pindexPrev->GetMedianTimePast()) {
...
bool TestBlockValidity(CValidationState& state, const CBlock& block, CBlockIndex* const pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (indexDummy.nHeight >= 655250 && fCheckPOW) {
